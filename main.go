...
func findLastJob(root Job) {

}

func dfs (root *Job, lenTree int) {
	...
	//本层逻辑
	for index, job := range root.deeps {
		//进入下一层
		dfs(job, lenTree+1)
		if index == len(root.deeps)/ 2 {
			_,err := jobMap[job.name]
			if err != false {

			}else {
				jobMap[root.name] = lenTree
			}
		}
	}
...
func FindTopK(A, B[]int, K int) (int, error) {
	...
	indexSum, flag := 0, 0
	if K > len(A) + len(B) {
		return 0,errors.New("长度设置有误")
	}
	for indexSum < K {
		if firstPtr < len(A) && secondPtr < len(B) {
			if A[firstPtr] > B[secondPtr] {
				firstPtr ++
				indexSum ++
				flag = 1
			} else if A[firstPtr] == B[secondPtr] {
				firstPtr ++
				indexSum ++
				flag = 1
			} else {
	...
	if flag == 0 {
		return A[firstPtr], nil
	}else {
		return B[secondPtr], nil
	}
...
func test01(root []JobTest)  {
	for index,job := range root {

	}
}


func minDepth(root *JobTest) int {
	if root == nil {
		return 0
	}
	var queue []*JobTest       // 查找队列
...
var listLen []int
func lenBinTree (root *Node) int {

	bfs(root,0)
	sort.Ints(listLen)
	return listLen[len(listLen) - 1]
}
func bfs (root *Node, len int) {
	//返回条件
	if root == nil {
		listLen = append(listLen, len)
	}

	//进入下一层
	bfs(root.left,len +1)
	bfs(root.right, len + 1)
	//本层逻辑

	//清理环境
}

func FindNum (numLen []int, target int) {
	res := [][]int{}
	sum := 0
	left, right := 0, 0
	for right < len(numLen) {
		if sum < target {
			sum += numLen[right]
			right ++
		} else if sum == target {
